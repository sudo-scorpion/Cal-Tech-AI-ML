...
def plot_sales_trends(data: pd.DataFrame, group_column: str, date_column: str, sales_column: str, title: str) -> None:
    """Plot sales trends over time for specified groups."""
    sns.set(style="whitegrid")
    
    # Aggregate data by month-end
    data_aggregated = data.groupby([group_column, pd.Grouper(key=date_column, freq='ME')])[sales_column].sum().reset_index()
    ...
    for group in unique_groups:
        ...
        plt.plot(group_data[date_column], group_data[sales_column], marker='o', label=group)
...
def plot_monthly_sales_trends(data: pd.DataFrame, date_column: str, sales_column: str, title: str) -> None:
    ...
    plt.figure(figsize=(10, 6))  # Smaller size
    ax = sns.lineplot(x=monthly_sales['Month'], y=monthly_sales[sales_column], marker='o')
    ax.set_title(title)
    ax.set_xlabel('Month')
    ax.set_ylabel('Sales')
    ...
    for x, y in zip(monthly_sales['Month'], monthly_sales[sales_column]):
        ax.annotate(f'{y:.0f}', xy=(x, y), xytext=(0, 5), textcoords='offset points', ha='center')

    plt.xticks(rotation=45)
    plt.grid(True)
    plt.show()

def plot_yearly_sales_trends(data: pd.DataFrame, date_column: str, sales_column: str, title: str) -> None:
